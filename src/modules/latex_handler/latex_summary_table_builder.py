...
logger = logging.getLogger("src.modules.latex_handler.LatexSummaryTableBuilder")
...
class LatexSummaryTableBuilder(LatexBaseTableBuilder):
    ...
    def generate_method_table_latex(self, data):
        ...
        for idx, (category, features, methods) in enumerate(
            ...
            row_span = len(features)  

            category_bold = f"\\textbf{{{category}}}" 
            features_bold = [f"{feature}" for feature in features]  
